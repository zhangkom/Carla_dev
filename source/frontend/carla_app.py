...
class CarlaApplication():
    def __init__(self, appName = "Carla2", libPrefix = None):
        ...
        if WINDOWS and stylesDir:
            hasStyle = os.path.exists(os.path.join(stylesDir, "styles", "carlastyle.dll"))

        # set style
        QApplication.setStyle("carla" if hasStyle else "fusion")
        ...
        self.fApp.setStyle("carla" if hasStyle else "fusion")

        if WINDOWS and hasStyle:
