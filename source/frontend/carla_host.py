...
#class HostWindow(QMainWindow, PluginEditParentMeta, metaclass=PyQtMetaClass):
    ...
    def slot_engineConfig(self):
        ...
        if bufferSize <= 0:
            bufferSize = self.host.get_buffer_size() if engineRunning else CARLA_DEFAULT_AUDIO_BUFFER_SIZE
        if sampleRate <= 0:
            sampleRate = int(self.host.get_sample_rate()) if engineRunning else CARLA_DEFAULT_AUDIO_SAMPLE_RATE
